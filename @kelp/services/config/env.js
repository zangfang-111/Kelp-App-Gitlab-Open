...
/* Use via `node -r @kelp/config/env path/to/file.js` */
...
function fixFilePaths(connectionString) {
  ...
  // similar; but we might be running it from somewhere other than `@kelp/*/`
  // (e.g. maybe `@kelp/*/dist/`). To solve this, we make the file path concrete
  // here.
  return connectionString.replace(/\.\.\/\.\.\/data\//g, resolve(__dirname, '../../data') + '/')
}
